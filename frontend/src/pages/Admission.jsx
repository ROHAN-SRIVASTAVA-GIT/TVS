...
const Admission = () => {
          ...
          if (window.PhonePe) {
            window.PhonePe.Checkout.init({
              ...
              checkout.onSuccess((data) => {
                ...
                verifyAndSubmitPayment(response.data.phonepeOrderId);
              }).onError((error) => {
  ...
  const verifyAndSubmitPayment = async (phonepeOrderId) => {
    try {
      ...
      if (verifyResponse.success && verifyResponse.data.paymentStatus === 'completed') {
        await submitAdmissionForm(phonepeOrderId);
      } else {
  ...
  useEffect(() => {
    const checkPaymentStatus = async () => {
      ...
      if (orderId || merchantOrderId || transactionId) {
        console.log('Checking payment status for:', orderId || merchantOrderId || transactionId);
        setLoading(true);
        ...
        try {
          ...
          } else if (code === 'PAYMENT_SUCCESS' || status === 'SUCCESS') {
            setPaymentSuccess(true);
            setSuccess('Payment successful! Your admission form has been submitted.');
          } else if (status === 'PENDING') {
            setError('Payment is pending. Please check status later.');
          } else {
        ...
        <div className="success-page">
          ...
          <div className="payment-receipt-preview">
            ...
            <div className="receipt-actions">
              <a 
                href={`${API_URL}/payments/receipt/new?studentName=${encodeURIComponent(formData.studentName)}&class=${formData.admissionClass}&amount=${feeStructure?.total_fee || 30000}&feeType=admission`}
                target="_blank"
